...
int load_exe(struct task_struct *task, char *path)
{
	...
	for(; count < ehdr->e_phnum; count++) 
		...
		if(phdr->p_type == 1) 
                     ...
                     {
			struct vm_area_struct *vma = allocateVMA(ehdr,phdr->p_vaddr,phdr->p_vaddr + phdr->p_memsz,phdr->p_flags,phdr->p_offset,NOTYPE,phdr->p_filesz);//addVMA();
...
allocateVMA(NULL,HEAP_START,HEAP_START + PAGE_SIZE,PERM_R | PERM_W,(uint64_t)NULL,HEAP,(uint64_t)NULL);

//stack

uint64_t *stack = (uint64_t *)get_phy_addr(USER_STACK_TOP);

allocateVMA(NULL,(uint64_t)stack + PAGE_SIZE,(uint64_t)stack,PERM_R | PERM_W,(uint64_t)NULL,STACK,(uint64_t)NULL);
